...
{    
    ...
    public partial class Form1 : Form
    {
        ...
        private const byte backward = (70); //same range forwards and back
        int cycleCount = 0;
        //PID control
        float kp = 49;
        float ki = 0.009f;
        float kd = 0.03f;
        int baseSpeed = 190; //slow work -> kp = 35, ki = 0.007, kd = 0.04, base Speed = 170 or 205 with kp = 33
                             // 33, 0.007, 0.06, base speed = 190
        float PID_influence = 1.2f;
        ...
        private async void reCallibrate_Click(object sender, EventArgs e) //recallibrate Lmin,Lmax etc automatically
        {
            ...
            reCallibrate.Text = "recallibrating... beep boop";
           ...
           // byte code;
            ...
            switch (currentMode)
            {
                ...
                case ControlMode.BangBang:
                    code = 127;
                    if ((int)Input1 > WHITE && (int)Input2 < WHITE) // turn left
                        code = forward + 10;
                    else if ((int)Input1 < WHITE && (int)Input2 > WHITE) // turn right
                        code = backward;
                    else if ((int)Input1 > WHITE && (int)Input2 > WHITE) // go nuts
                        code = forward + 10;
                    else if ((int)Input1 < WHITE && (int)Input2 < WHITE) // cautiously go nuts
                        code = forward - 15;
                    break;
        ...
        private void rightContControl() //right continuous controls
        {
            ...
            switch (currentMode)
            {
                ...
                case ControlMode.BangBang:
                    if ((int)Input1 > WHITE & (int)Input2 < WHITE) // left turn
                        code = backward; // right motor stop
                    else if ((int)Input1 < WHITE & (int)Input2 > WHITE) // right turn
                        code = forward; // right motor forward
                    else if ((int)Input1 > WHITE & (int)Input2 > WHITE)// all steam ahead - both see board
                        code = forward;
                    else if ((int)Input1 < WHITE & (int)Input2 < WHITE)// both see white (weird)
                        code = forward - 15;
                    break;
